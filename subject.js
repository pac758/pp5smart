...
function getGlobalSettingsFixed() {
  Logger.log('🔧 ใช้ getGlobalSettingsFixed() เพื่อ bypass ปัญหา settings.gs');
  ...
  try {
    ...
    if (result.exists && Object.keys(result.map).length > 0) {
      Logger.log('✅ ได้ข้อมูลจาก _readKV_:', Object.keys(result.map).length, 'คีย์');
      Logger.log('👨‍💼 ชื่อผู้อำนวยการ:', result.map['ชื่อผู้อำนวยการ']);
      return result.map; // คืนข้อมูลดิบจาก _readKV_
    }
    
    Logger.log('⚠️ ไม่พบข้อมูล จะใช้ค่าเริ่มต้น');
    return getDefaultSettingsForSubject();
    
  } catch (err) {
    Logger.log('❌ getGlobalSettingsFixed:', err);
    return getDefaultSettingsForSubject();
  }
}
...
function getTeacherFromSubjectFixed(subjectName, grade, subjectCode) {
  try {
    ...
    Logger.log('❌ getTeacherFromSubjectFixed:', err);
    return "ครูผู้สอน";
  }
}
...
function _extractScoreDataFixed_(sheet, term) {
  try {
    ...
    Logger.log(`📊 กำลังดึงข้อมูลจากชีต: "${sheetName}"`);
    
    const data = sheet.getDataRange().getValues();
    Logger.log(`📏 ชีตมี ${data.length} แถว, ${data[0] ? data[0].length : 0} คอลัมน์`);
    ...
    Logger.log('🔍 Debug ข้อมูลในชีต:');
    Logger.log(`  แถวที่ 0 (header): [${data[0].slice(0, 10).map(v => `"${v}"`).join(', ')}...]`);
    Logger.log(`  แถวที่ 1: [${data[1].slice(0, 10).map(v => `"${v}"`).join(', ')}...]`);
    Logger.log(`  แถวที่ 2: [${data[2].slice(0, 10).map(v => `"${v}"`).join(', ')}...]`);
    Logger.log(`  แถวที่ 3: [${data[3].slice(0, 10).map(v => `"${v}"`).join(', ')}...]`);
    if (data.length > 4) {
      Logger.log(`  แถวที่ 4 (นักเรียนคนแรก): [${data[4].slice(0, 10).map(v => `"${v}"`).join(', ')}...]`);
    }
    ...
    for (let r = 3; r < Math.min(data.length, 8); r++) {
      ...
      if (row[0] && row[1] && row[2]) {
        studentStartRow = r;
        Logger.log(`✅ พบข้อมูลนักเรียนเริ่มที่แถว: ${r}`);
        break;
      }
    }
    ...
    for (let r = studentStartRow; r < data.length; r++) {
      ...
      Logger.log(`🔍 แถว ${r}: ที่="${row[0]}", รหัส="${row[1]}", ชื่อ="${row[2]}"`);

      // ✅ ตรวจสอบข้อมูลคะแนนในคอลัมน์ต่างๆ
      Logger.log(`    ภาค1: ระหว่าง[13]="${row[13]}", ปลาย[14]="${row[14]}", รวม[15]="${row[15]}", เกรด[16]="${row[16]}"`);
      Logger.log(`    ภาค2: ระหว่าง[27]="${row[27]}", ปลาย[28]="${row[28]}", รวม[29]="${row[29]}", เกรด[30]="${row[30]}"`);
      ...
      Logger.log(`✅ เพิ่มนักเรียน: ${student.name} (${student.studentId})`);
    }

    Logger.log(`✅ ดึงข้อมูลได้ทั้งหมด ${students.length} คน จากชีต "${sheetName}"`);
    ...
    Logger.log('❌ _extractScoreDataFixed_ error:', err);
...
function generateSubjectScorePDFFixed(subjectName, subjectCode, grade, classNo, term = 'both') {
  try {
    Logger.log(`🔨 generateSubjectScorePDFFixed: ${subjectName}, ${grade}/${classNo}, term: ${term}`);
    ...
    try {
      ...
      Logger.log('ไม่สามารถโหลดโลโก้ได้:', e.message);
    }
...
function getPp6ReportDataComplete(studentId, term = 'both') {
  try {
    Logger.log(`📊 รวบรวมข้อมูล ปพ.6 สำหรับ: ${studentId}`);
    ...
    Logger.log('❌ Error in getPp6ReportDataComplete:', error);
...
function getStudentPersonalInfo(studentId) {
  try {
    ...
    Logger.log('❌ Error in getStudentPersonalInfo:', error);
  ...
  try {
    ...
    scoreSheets.forEach(sheet => {
      try {
        ...
        Logger.log(`ข้ามชีต ${sheet.getName()}: ${e.message}`);
      }
    });
    ...
    Logger.log('Error in getStudentAllSubjectResults:', error);
