...
// User-facing final override with clearer already-exists messaging.
...
function AY_prepareShouldCopyRow_(baseName, row, headers, targetYear) {
  if (AY_prepareCanCopyRow_(row, headers, targetYear)) return true;
  if (baseName !== 'Students') return false;

  var yearIdx = (typeof AY_findColumn_ === 'function')
    ? AY_findColumn_(headers, ['academic_year', 'academicyear', 'academic year', 'year', (typeof AY_KEY_THAI !== 'undefined' ? AY_KEY_THAI : 'ปีการศึกษา')])
    : -1;
  var rowYear = yearIdx >= 0 ? AY_archiveTargetYear_(row[yearIdx]) : '';

  // Legacy Students rows often have blank academic_year. For the current
  // working Students_YEAR sheet, active blank-year rows are treated as current.
  return !rowYear && AY_prepareIsActiveStudentRow_(row, headers);
}
...
function AY_prepareNormalizeRowForTarget_(baseName, row, headers, targetYear) {
  ...
  if (baseName === 'Students') {
    var yearIdx = (typeof AY_findColumn_ === 'function')
      ? AY_findColumn_(headers, ['academic_year', 'academicyear', 'academic year', 'year', (typeof AY_KEY_THAI !== 'undefined' ? AY_KEY_THAI : 'ปีการศึกษา')])
      : -1;
    if (yearIdx >= 0) out[yearIdx] = String(targetYear);
  }
  return out;
}
...
function AY_preparePlanForSheet_(ss, baseName, targetYear) {
  ...
  for (var r = 1; r < data.length; r++) {
    var row = data[r];
    if (yearIdx >= 0 && !AY_archiveTargetYear_(row[yearIdx])) item.blankYearRows++;
    if (AY_prepareShouldCopyRow_(baseName, row, headers, targetYear)) item.explicitTargetYearRows++;
  }
  return item;
}

// Fast final override. This must be the last createCurrentYearWorkingSheets
// definition in the file because Apps Script uses the last function declaration.
function createCurrentYearWorkingSheets(targetYear) {
  var lock = LockService.getScriptLock();
  if (!lock.tryLock(5000)) {
    return { success: false, message: 'ระบบยังทำงานคำสั่งก่อนหน้าอยู่ กรุณารอสักครู่แล้วกดใหม่' };
  ...
  try {
    ...
    Logger.log('createCurrentYearWorkingSheets fast final error: ' + e.message + '\n' + (e.stack || ''));
    return { success: false, message: e.message };
  } finally {
    try { lock.releaseLock(); } catch (_) {}
  }
...
// Final user-facing override. This is intentionally after the last legacy
// function so Apps Script uses this clearer and faster implementation.
function createCurrentYearWorkingSheets(targetYear) {
  var lock = LockService.getScriptLock();
  if (!lock.tryLock(5000)) {
    return { success: false, message: 'ระบบยังทำงานคำสั่งก่อนหน้าอยู่ กรุณารอสักครู่แล้วกดใหม่' };
...
function previewCurrentYearWorkingSheets(targetYear) {
  try {
    ...
    sheets.forEach(function(item) {
      if (!item.sourceExists) totals.missingSource++;
      else if (item.targetExists) totals.alreadyExists++;
    ...
    return {
      ...
      sheets: sheets,
      totals: totals,
      note: 'จะสร้างชีตรายปี _' + year + ' โดย Students จะคัดลอกนักเรียนที่ยังใช้งานอยู่และเติม academic_year ให้เป็นปีนี้ ส่วนชีตคะแนน/เวลาเรียนจะคัดลอกเฉพาะแถวที่มี academic_year ตรงปีนี้'
    };
  } catch (e) {
...
function createCurrentYearWorkingSheets(targetYear) {
  ...
  try {
    ...
    Logger.log('createCurrentYearWorkingSheets fast error: ' + e.message + '\n' + (e.stack || ''));
    return { success: false, message: e.message };
  } finally {
    try { lock.releaseLock(); } catch (_) {}
  }
...
function AY_preparePlanForSheet_(ss, baseName, targetYear) {
  ...
  // Preview must stay quick. If the yearly target already exists, there is
  // nothing to scan in the large legacy source sheet.
  ...
  if (baseName !== 'Students') {
    ...
    var yearValues = source.getRange(2, yearIdx + 1, source.getLastRow() - 1, 1).getValues();
    for (var y = 0; y < yearValues.length; y++) {
      if (!AY_archiveTargetYear_(yearValues[y][0])) item.blankYearRows++;
      if (AY_archiveTargetYear_(yearValues[y][0]) === String(targetYear)) item.explicitTargetYearRows++;
    }
    return item;
  }
  ...
  try {
    ...
    preview.sheets.forEach(function(item) {
      ...
      try {
        ...
        for (var r = 1; r < data.length; r++) {
          if (AY_prepareShouldCopyRow_(item.baseName, data[r], headers, preview.targetYear)) {
            rows.push(AY_prepareNormalizeRowForTarget_(item.baseName, data[r], headers, preview.targetYear));
          }
        }

        var target = source.copyTo(ss);
        target.setName(item.target);
        if (target.getLastRow() > 1) {
          target.deleteRows(2, target.getLastRow() - 1);
        }
        if (rows.length > 1) {
          target.getRange(2, 1, rows.length - 1, rows[0].length).setValues(rows.slice(1));
        }
        created.push(item.target);
        copiedRows += rows.length - 1;
      } catch (e) {
        errors.push(item.source + ' → ' + item.target + ': ' + e.message);
      }
    });
    ...
    Logger.log('createCurrentYearWorkingSheets error: ' + e.message + '\n' + (e.stack || ''));
    return { success: false, message: e.message };
  } finally {
    try { lock.releaseLock(); } catch (_) {}
  }
...
// Fast final override. Keep this after all legacy createCurrentYearWorkingSheets
// declarations so creating Students_<year> does not use slow sheet copyTo().
function createCurrentYearWorkingSheets_fastOld_(targetYear) {
  var lock = LockService.getScriptLock();
  if (!lock.tryLock(5000)) {
    return { success: false, message: 'ระบบยังทำงานคำสั่งก่อนหน้าอยู่ กรุณารอสักครู่แล้วกดใหม่' };
  }
