...
function generateCharacteristicAssessmentPDF(grade, classNo) {
  try {
    var validated = validateGradeAndClass_(grade, classNo);
...
// ----- G2. Activity PDF -----
function generateActivityAssessmentPDF(grade, classNo) {
  try {
    var validated = validateGradeAndClass_(grade, classNo);
...
function generateSubjectScoreAssessmentPDF(grade, classNo) {
  try {
    var validated = validateGradeAndClass_(grade, classNo);
...
function exportCompetencyReport(grade, classNo) {
  try {
    var validated = validateGradeAndClass_(grade, classNo);
...
function exportCompetencySummaryReport(grade, classNo) {
  try {
    var validated = validateGradeAndClass_(grade, classNo);
    var studentsData = getStudentsForCompetency(validated.grade, validated.classNo);
    if (!studentsData || studentsData.length === 0) throw new Error('ไม่พบข้อมูลนักเรียน');
...
function createCharacteristicAssessmentHTML(students, grade, classNo, year, school, director, teacherName, logoBase64) {
  ...
  students.forEach(function(stu, i) {
    ...
    rows += '<td class="center font-bold">' + (total !== '' ? total : '-') + '</td>';
    rows += '<td class="center font-bold">' + (avg !== '' ? avg : '-') + '</td>';
    rows += '<td class="center font-bold">' + result + '</td></tr>';
  });
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;600;700&display=swap" rel="stylesheet"><style>'
    + '@page{size:A4 portrait;margin:1.5cm 1cm}body{font-family:"Sarabun",sans-serif;font-size:11pt;line-height:1.2;color:#000}.header{text-align:center;margin-bottom:8px}.logo{width:60px;height:60px;object-fit:contain;margin-bottom:5px}.title{font-size:15pt;font-weight:bold}.school-name{font-size:13pt;text-align:center;margin-top:3px}.info-line{text-align:center;margin:5px 0 10px 0;font-size:12pt}table{width:100%;border-collapse:collapse;margin-bottom:10px}th,td{border:1px solid #000;padding:3px 2px;font-size:10pt;vertical-align:middle}th{background-color:#fef9e7;font-weight:bold;color:#000}.vertical-text{writing-mode:vertical-rl;text-orientation:mixed;height:100px;white-space:nowrap;padding:5px 2px}.center{text-align:center}.left{text-align:left}.font-bold{font-weight:bold}'
    + '.summary-box{margin-top:15px;padding:8px;border:1px solid #000;background-color:#f8f9fa}.summary-row{display:flex;justify-content:space-around;margin:5px 0}.summary-item{text-align:center;flex:1}.summary-label{font-size:11pt;margin-bottom:3px}.summary-value{font-size:14pt;font-weight:bold}.excellent{color:#28a745}.good{color:#007bff}.pass{color:#ffc107}.improve{color:#dc3545}'
    + '.legend{margin-top:8px;font-size:9pt;text-align:center}.footer{margin-top:25px;width:100%;display:table;page-break-inside:avoid}.sign-left,.sign-right{display:table-cell;width:50%;text-align:center;vertical-align:top}.sign-line{margin-bottom:4px;font-size:11pt}'
    + '</style></head><body>'
...
function createActivityAssessmentHTML_(students, grade, classNo, year, school, director, teacherName, logoBase64) {
  ...
  students.forEach(function(stu, i) {
    ...
    rows += '<tr><td class="center">' + (i + 1) + '</td><td class="left name-cell">' + stu.name + '</td>'
      ...
      + '<td class="center font-bold ' + getRC(overall) + '">' + overall + '</td></tr>';
  });
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@400;500;600;700&display=swap" rel="stylesheet"><style>'
    + '@page{size:A4;margin:1cm 1.2cm}body{font-family:"Sarabun",sans-serif;font-size:11pt;line-height:1.2;color:#000;margin:0;padding:0}.header{text-align:center;margin-bottom:8px}.logo{width:50px;height:50px;object-fit:contain;margin-bottom:5px}.title{font-size:16pt;font-weight:700;margin-bottom:3px}.subtitle{font-size:14pt;font-weight:600}.info-box{text-align:center;margin-bottom:10px;font-size:13pt;font-weight:500}table{width:100%;border-collapse:collapse;margin-bottom:8px}th,td{border:1px solid #000;padding:4px 3px;font-size:10pt;vertical-align:middle}th{background-color:#198754;color:white;font-weight:600;height:32px}.center{text-align:center}.left{text-align:left;padding-left:5px!important}.font-bold{font-weight:600}.name-cell{font-size:9pt;white-space:nowrap}.pass{color:#198754}.not-pass{color:#dc3545}'
    + '.summary-inline{margin:10px 0;padding:8px 15px;border:1.5px solid #198754;border-radius:5px;background-color:#f8fff8;text-align:center;font-size:11pt}.summary-inline .value{font-weight:700;font-size:14pt;margin:0 3px}.legend-inline{margin:8px 0;font-size:10pt;text-align:center}'
    + '.footer{margin-top:20px;width:100%}.footer table{border:none;margin:0}.footer td{border:none;width:50%;text-align:center;vertical-align:top;padding:0 10px}.sign-line{margin-bottom:5px;font-size:11pt}'
    + '</style></head><body>'
...
function createOfficialReportHTML(students, grade, classNo, year, school, director, teacherName, logoBase64) {
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;700&display=swap" rel="stylesheet"><style>'
    + '@page{size:A4;margin:2cm 1.5cm}body{font-family:"Sarabun",sans-serif;font-size:14pt;line-height:1.3;color:#000}.header{text-align:center;margin-bottom:15px}.logo{width:60px;height:60px;object-fit:contain;margin-bottom:8px}.title{font-size:18pt;font-weight:bold}.subtitle{font-size:14pt;font-weight:bold;margin-top:5px}.info-box{text-align:center;margin-bottom:15px;font-size:14pt;font-weight:bold}table{width:100%;border-collapse:collapse;margin-bottom:20px}th,td{border:1px solid #000;padding:4px 2px;font-size:12pt;vertical-align:middle}th{background-color:#f0f0f0;font-weight:bold;height:40px}.center{text-align:center}.left{text-align:left}.font-bold{font-weight:bold}.name-cell{font-size:12pt;white-space:nowrap}'
    + '.footer{margin-top:40px;width:100%;display:table;page-break-inside:avoid}.sign-left,.sign-right{display:table-cell;width:50%;text-align:center;vertical-align:top}.sign-line{margin-bottom:5px;font-size:12pt;white-space:nowrap}'
    + '</style></head><body>'
...
function createCompetencyDetailedHTML(students, grade, classNo, year, school, director, teacherName, logoBase64) {
  ...
  students.forEach(function(stu, i) {
    ...
    rows += '<td class="center font-bold">' + totalSum + '</td><td class="center font-bold">' + totalAvg + '</td><td class="center font-bold">' + result + '</td></tr>';
  });

  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;600;700&display=swap" rel="stylesheet"><style>'
    + '@page{size:A4 landscape;margin:1.5cm}body{font-family:"Sarabun",sans-serif;font-size:12pt;line-height:1.3;color:#000}.header{text-align:center;margin-bottom:10px}.logo{width:50px;height:50px;object-fit:contain;vertical-align:middle;margin-right:10px}.title{font-size:16pt;font-weight:bold;display:inline;vertical-align:middle}.school-name{font-size:14pt;text-align:center;margin-top:5px}.info-line{text-align:center;margin:8px 0 15px 0;font-size:13pt}table{width:100%;border-collapse:collapse;margin-bottom:10px}th,td{border:1px solid #000;padding:4px 2px;font-size:11pt;vertical-align:middle}th{background-color:#f0f0f0;font-weight:bold}.center{text-align:center}.left{text-align:left}.font-bold{font-weight:bold}'
    + '.legend{margin-top:10px;font-size:10pt}.footer{margin-top:40px;width:100%}.footer table{border:none}.footer td{border:none;width:50%;text-align:center;vertical-align:top}.sign-line{margin-bottom:5px}'
    + '</style></head><body>'
...
function createCompetencySummaryHTML(students, grade, classNo, year, school, director, teacherName, logoBase64) {
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;600;700&display=swap" rel="stylesheet"><style>'
    + '@page{size:A4;margin:2cm 1.5cm}body{font-family:"Sarabun",sans-serif;font-size:14pt;line-height:1.4;color:#000}.header{text-align:center;margin-bottom:8px}.logo{width:55px;height:55px;object-fit:contain;vertical-align:middle;margin-right:10px}.title{font-size:18pt;font-weight:bold;display:inline;vertical-align:middle}.school-name{font-size:16pt;text-align:center;margin-top:5px}.info-line{text-align:center;margin:10px 0 15px 0;font-size:15pt}table{width:100%;border-collapse:collapse;margin-bottom:15px}th,td{border:1px solid #000;padding:8px 5px;font-size:14pt;vertical-align:middle}th{background-color:#f0f0f0;font-weight:bold}.center{text-align:center}.left{text-align:left;padding-left:10px}.font-bold{font-weight:bold}.total-row{background-color:#e8f5e9}'
    + '.summary-line{text-align:center;margin:20px 0;font-size:15pt}.summary-item{display:inline-block;margin:0 25px}.summary-label{font-weight:normal}.summary-value{font-weight:bold;font-size:18pt;margin-left:5px}.excellent{color:#28a745}.good{color:#007bff}.pass{color:#ff9800}.improve{color:#dc3545}'
    + '.footer{margin-top:50px;width:100%}.footer table{border:none}.footer td{border:none;width:50%;text-align:center;vertical-align:top}.sign-line{margin-bottom:8px;white-space:nowrap}'
    + '</style></head><body>'
