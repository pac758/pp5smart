...
function clearLoginSession(){ try { PropertiesService.getUserProperties().deleteAllProperties(); } catch(e) {} }
...
function verifyLogin(username, password){
  ...
  try{
    ...
    // ✅ ถ้า role ว่าง ให้ใช้ 'teacher' เป็นค่าเริ่มต้น
    var userRole = String(row[roleIdx]||'').trim();
    if (!userRole) {
...
function addNewUser(payload) {
  ...
  try {
    ...
    newRow[pwIdx] = hashPassword_(password);
    ...
    // บันทึก student_ids สำหรับ parent
    var sidIdx = __idx__(map, ['student_ids','studentids','linked_students','รหัสนักเรียน'], -1);
    if (sidIdx >= 0 && payload.studentIds) {
      newRow[sidIdx] = String(payload.studentIds).trim();
    }
