...
function pp5s_coverDoc_(meta, studentCount, subjectSummary, reading, attributes, activity) {
  ...
  var actKw_ = ['กิจกรรม', 'แนะแนว', 'ลูกเสือ', 'เนตรนารี', 'ชุมนุม', 'เพื่อสังคม', 'ชมรม'];
  var filteredCount = (subjectSummary || []).filter(function(s) {
    var n = (s.name || '').trim();
    return !actKw_.some(function(kw) { return n.indexOf(kw) !== -1; });
  }).length;
  var compactCover = (filteredCount >= 14);

  // ตั้งค่าหน้า A4 portrait
  body.setPageWidth(595.28).setPageHeight(841.89)
    .setMarginTop(compactCover ? 26 : 36)
    .setMarginBottom(compactCover ? 18 : 28)
    .setMarginLeft(compactCover ? 42 : 50)
    .setMarginRight(compactCover ? 20 : 28);
  ...
  if (meta.logoDataUri) {
    try {
      ...
      if (match) {
        ...
        var logoSize = compactCover ? 40 : 50;
        firstPara.appendInlineImage(imgBlob).setWidth(logoSize).setHeight(logoSize);
        usedFirstPara = true;
      }
  ...
  if (!usedFirstPara) {
    ...
    firstPara.setAlignment(DocumentApp.HorizontalAlignment.CENTER);
    stylePara(firstPara, compactCover ? 17 : 18, true);
    ...
    stylePara(h1, compactCover ? 17 : 18, true);
  ...
  stylePara(h2, compactCover ? 15 : 16, true);
  ...
  stylePara(h4, compactCover ? 15 : 16, true);
  h4.setLineSpacing(1.0).setSpacingBefore(compactCover ? 1 : 3).setSpacingAfter(1);
  ...
  stylePara(h5, compactCover ? 13 : 14, true);
  h5.setLineSpacing(1.0).setSpacingBefore(1).setSpacingAfter(compactCover ? 2 : 4);
  ...
  var sorted = (subjectSummary || []).filter(function(s) {
    var n = (s.name || '').trim();
  ...
  ['ที่', 'ชื่อวิชา', 'ระดับผลการเรียนรายวิชา (คน)', '', '', '', '', '', '', ''].forEach(function(h) {
    ...
    styleCell(cell, compactCover ? 11 : 12, true);
  ...
  sorted.forEach(function(s, i) {
    ...
    vals.forEach(function(v, ci) {
      ...
      styleCell(cell, compactCover ? 11 : 12, false);
  ...
  body.appendParagraph('').setLineSpacing(1.0).setSpacingBefore(0).setSpacingAfter(0);
  ...
  stylePara(evalTitle, compactCover ? 13 : 14, true);
  evalTitle.setLineSpacing(1.0).setSpacingBefore(0).setSpacingAfter(compactCover ? 1 : 2);

  var pct = function(n) { return studentCount > 0 ? ((n || 0) / studentCount * 100).toFixed(1) : '0.0'; };

  var evalTable = body.appendTable();

  // header row 1: 3 กลุ่ม ในแต่ละกลุ่ม 3 คอลัมน์ — ใส่ชื่อเต็ม
  var eh1 = evalTable.appendTableRow();
  ['การอ่าน คิดวิเคราะห์และเขียน', '', '', 'คุณลักษณะอันพึงประสงค์', '', '', 'กิจกรรมพัฒนาผู้เรียน', '', ''].forEach(function(h) {
    var cell = eh1.appendTableCell(h);
    styleCell(cell, compactCover ? 11 : 11, true);
    cell.getChild(0).asParagraph().setAlignment(DocumentApp.HorizontalAlignment.CENTER);
    cell.setBackgroundColor('#f0f0f0');
    cell.setPaddingTop(1).setPaddingBottom(1).setPaddingLeft(1).setPaddingRight(1);
  });
  ...
  [
    ...
    rowData.forEach(function(v, ci) {
      var cell = row.appendTableCell(v);
      styleCell(cell, compactCover ? 11 : 11, false);
      var align = DocumentApp.HorizontalAlignment.CENTER;
  ...
  body.appendParagraph('').setLineSpacing(1.0).setSpacingBefore(compactCover ? 2 : 4).setSpacingAfter(0);
  ...
  [['', ''], ['☐ เห็นควรอนุมัติ  ☐ เห็นควรปรับปรุง', ''], ['☐ อนุมัติ  ☐ ไม่อนุมัติ', '']].forEach(function(pair) {
    ...
    styleCell(cell, compactCover ? 10 : 12, false);
    cell.getChild(0).asParagraph().setAlignment(DocumentApp.HorizontalAlignment.CENTER);
    cell.setPaddingTop(compactCover ? 0 : 1).setPaddingBottom(compactCover ? 0 : 1);
  });
  ...
  signLines.forEach(function(lines) {
    ...
    lines.forEach(function(line, li) {
      ...
      para.editAsText().setFontFamily('Sarabun').setFontSize(compactCover ? 10 : 12).setBold(false);
      para.setLineSpacing(compactCover ? 1.0 : 1.15).setSpacingBefore(0).setSpacingAfter(0);
    });
    cell.setPaddingTop(compactCover ? 1 : 3).setPaddingBottom(compactCover ? 1 : 3);
  });
  ...
  try {
    ...
    // evalTable = ตาราง 9 columns (ตารางประเมินบูรณาการ)
    for (var ti2 = 0; ti2 < tables.length; ti2++) {
      var tbl2 = tables[ti2].table;
      if (tbl2.columns === 9 && tbl2.rows >= 3) {
        var tsi2 = tables[ti2].startIndex;
        // แถว 1 (row 0): merge 3 กลุ่ม × 3 คอลัมน์
        mergeReqs.push({ mergeTableCells: { tableRange: { tableCellLocation: { tableStartLocation: { index: tsi2 }, rowIndex: 0, columnIndex: 0 }, rowSpan: 1, columnSpan: 3 } } });
        mergeReqs.push({ mergeTableCells: { tableRange: { tableCellLocation: { tableStartLocation: { index: tsi2 }, rowIndex: 0, columnIndex: 3 }, rowSpan: 1, columnSpan: 3 } } });
        mergeReqs.push({ mergeTableCells: { tableRange: { tableCellLocation: { tableStartLocation: { index: tsi2 }, rowIndex: 0, columnIndex: 6 }, rowSpan: 1, columnSpan: 3 } } });
        break;
      }
    }
...
function pp5s_dailyAttendance_(ss, meta, tableData, monthName, yearBE, semester) {
  ...
  students.forEach(function(stu, i) {
    ...
    sheet.getRange(r, 1).setHorizontalAlignment('center');
    sheet.getRange(r, 2).setHorizontalAlignment('center');
    sheet.getRange(r, 4, 1, numDays).setHorizontalAlignment('center').setFontFamily('Arial').setFontWeight('normal').setFontSize(9);
...
async function exportPp5FullBookSheets(grade, classNo, parts) {
  try {
    ...
    if (parts.part1) {
      pp5fb_setProgress_(16, 4, 'ดึงข้อมูลสำหรับปก...');
